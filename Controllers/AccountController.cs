...
namespace Group25_Final_Project.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            //if order data is valid, create a new user on the database
            if (ModelState.IsValid)
            {
                ...
                AppUser newUser = new AppUser
                {
                    ...
                    //Done: Add the rest of the custom user fields here
                ...
                DateTime? Agelimit = new DateTime(12, 4, 2007);
                //If the add user operation was successful, we need to do a few more things

                //Figure out how to compare the email that is created to all emails so the email can be unique


                if (result.Succeeded && rvm.Birthday > Agelimit)
                {
                    //Done: Add user to desired role. This example adds the user to the customer role
        ...
        public IActionResult Index()
        {
            ...
            ivm.UserStreet = user.Address;
            ivm.UserState = user.State;
            ivm.UserCity = user.City;
            ivm.UserZipCode = user.ZipCode;
        ...
        public ActionResult ChangeStreet()
        {
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ChangeStreet(ChangeStreetViewModel csvm)
        {
            //if user forgot a field, send them back to 
            //change password page to try again
            if (ModelState.IsValid == false)
            ...
            userLoggedIn.Address = csvm.Street;
            userLoggedIn.State = csvm.State;
            userLoggedIn.City = csvm.City;
            userLoggedIn.ZipCode = csvm.ZipCode;
